-- 1. Cross-currency transfers: store the received amount separately.
ALTER TABLE money_transactions
  ADD COLUMN IF NOT EXISTS received_amount numeric(12,2);

-- 2. Balance corrections: add 'correction' to the allowed transaction types.
ALTER TABLE money_transactions
  DROP CONSTRAINT IF EXISTS money_transactions_type_check;
ALTER TABLE money_transactions
  ADD CONSTRAINT money_transactions_type_check
  CHECK (type IN ('income', 'expense', 'transfer', 'correction'));

-- 3. Updated running balance function (received_amount for transfers + correction type).
